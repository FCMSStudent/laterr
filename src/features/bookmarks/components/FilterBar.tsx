    ...
    <Drawer>
      ...
      <DrawerContent className="max-h-[85vh]">
        ...
        <div className="overflow-y-auto p-4 space-y-6">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {CATEGORY_OPTIONS.map(category => (
                <Button key={category.value} variant={selectedTag === category.value ? "default" : "outline"} size="sm" onClick={() => onTagSelect(category.value)} className="min-h-[44px]">
                  {category.label}
                </Button>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="flex flex-col gap-2">
              {[
                ...
                <Button key={option.value} variant={selectedSort === option.value ? "default" : "outline"} size="sm" onClick={() => onSortChange(option.value)} className="min-h-[44px] justify-start">
                  {option.label}
                </Button>
              ))}
            </div>
          ...
          {/* Clear All */}
          {hasActiveFilters && (
            <Button variant="destructive" size="sm" onClick={onClearAll} className="w-full min-h-[44px]">
              Clear All Filters
            </Button>
          )}
        </div>
  ...
  if (isMobile && hideMobileControls) {
    ...
    return (
      <div className="flex flex-wrap gap-2 items-center justify-center py-2">
        ...
        {selectedTag && (
          <Badge variant="secondary" className="cursor-pointer hover:bg-destructive/10 premium-transition text-xs" onClick={() => onTagSelect(null)}>
            #{selectedTag}
        ...
        {selectedTypeFilter && (
          <Badge variant="secondary" className="cursor-pointer hover:bg-destructive/10 premium-transition text-xs flex items-center gap-1" onClick={() => onTypeFilterChange(null)}>
              ...
              <div className="flex items-center gap-2">
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="start" className="w-48">
                    ...
                    {CATEGORY_OPTIONS.map(category => (
                      <DropdownMenuItem key={category.value} onClick={() => onTagSelect(category.value)}>
                        {category.label}
                      </DropdownMenuItem>
                    ))}
                  </DropdownMenuContent>
