...
const EnhancedTextarea = React.forwardRef<HTMLTextAreaElement, EnhancedTextareaProps>(
  ({ 
    ...
    const textareaClasses = cn(
      "flex min-h-[80px] w-full rounded-lg border bg-background px-4 py-3 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 transition-all duration-300 shadow-sm focus-visible:shadow-md",
          ...
          {showCharacterCount && maxLength && (
            <p 
              className={cn(
                ...
                isNearLimit && !error && "text-warning",
                showError && "text-destructive",
                !isNearLimit && !showError && "text-muted-foreground"
              )}
              aria-live="polite"
              aria-atomic="true"
            >
              {remaining !== null && remaining < 0 ? (
                <span>Over limit by {Math.abs(remaining)}</span>
              ) : (
