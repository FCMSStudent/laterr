...
export const DetailViewModal = ({ open, onOpenChange, item, onUpdate }: DetailViewModalProps) => {
  ...
  useEffect(() => {
    const generateSignedUrlForContent = async () => {
      if (!item?.content) {
        setSignedUrl(null);
        return;
      }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const message = handleError('saving changes', error, "Failed to save changes.");
      toast.error(message);
    } finally {
      setSaving(false);
    }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const message = handleError('deleting item', error, "Failed to delete item.");
      toast.error(message);
    } finally {
      setDeleting(false);
    }
