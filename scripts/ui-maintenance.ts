...
/**
 * Scans for accessibility issues (alt attributes, form labels, icon-only buttons)
 */
function scanAccessibility(): UIIssue[] {
  ...
  try {
    ...
    for (const file of files) {
      ...
      // 1. Scan for missing alt attributes
      ...
      // 2. Scan for icon-only buttons missing aria-label
      ...
      // 3. Scan for form elements missing labels
      const isSharedUI = file.includes('src/shared/components/ui');
      if (!isSharedUI) {
...
function scanInconsistentStyles(): UIIssue[] {
  const issues: UIIssue[] = [];
  try {
    // Scan for hardcoded pixel values in common Tailwind classes
...
function runRuntimeAudit(): UIIssue[] {
  ...
  try {
    // Run only chromium to save time, and pipe output to avoid cluttering logs
    ...
    if (fs.existsSync(resultsPath)) {
      ...
      // Find the first '{' in case there's npm noise before the JSON
      const jsonStart = resultsText.indexOf('{');
      if (jsonStart === -1) return [];

      const results: PlaywrightResult = JSON.parse(resultsText.substring(jsonStart));
            ...
            for (const result of test.results || []) {
              if (result.status === 'failed' || result.status === 'timedOut') {
                ...
                for (const fail of failures) {
                  // Strip ANSI color codes and sanitize paths
...
function generateMarkdownReport(issues: UIIssue[]) {
  ...
  for (const sev of severities) {
    ...
    if (sevIssues.length > 0) {
      ...
      // Group identical issues to make report cleaner
      const seen = new Set<string>();
      for (const issue of sevIssues) {
        const key = `${issue.type}-${issue.description}-${issue.file}-${issue.line}`;
...
async function main() {
  ...
  if (process.argv.includes('--fix')) {
    ...
    // Re-scan after fixes to see what's left
    const postFixAccessibility = scanAccessibility();
    const postFixColors = scanHardcodedColors();
    allIssues = [...postFixAccessibility, ...postFixColors, ...buttonIssues, ...styleIssues];
  }
