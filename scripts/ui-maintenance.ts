...
/**
 * Scans for accessibility issues (missing alt attributes, icon-only buttons without labels, form elements without labels)
 */
function scanAccessibilityIssues(): UIIssue[] {
  ...
  try {
    ...
    for (const file of files) {
      ...
      // 3. Form elements missing labels
...
/**
 * Scans for Button components that use custom Tailwind classes to override design system styles
 */
  ...
  try {
    ...
    for (const file of files) {
      ...
      while ((match = buttonRegex.exec(content)) !== null) {
        ...
        // Look for custom overrides: bg-, text-, p-, m-
        const overrides = classes.split(' ').filter(cls =>
          cls.startsWith('bg-') ||
          cls.startsWith('text-') ||
          cls.startsWith('p-') ||
          cls.startsWith('m-')
        ...
        if (overrides.length > 0) {
          ...
          issues.push({
            type: 'styling',
            severity: 'low',
            description: `Button component has custom styling overrides: ${overrides.join(', ')}`,
            file,
            line: lineNum,
            autoFixable: false
          });
            ...
            for (const result of test.results || []) {
              if (result.status === 'failed') {
                ...
                for (const fail of failures) {
                  ...
                  const failureMessage = (fail.message || 'Unknown failure').replace(/\x1b\[[0-9;]*m/g, '');
                  issues.push({
                    type: 'runtime',
...
async function sendSlackNotification(issues: UIIssue[]) {
  ...
  if (criticalIssues.length > 0) {
    mentionLeads = '\n🚨 *Critical Design/Accessibility Blockers Found!* tagging ';
    mentionLeads += designLeadId ? `<@${designLeadId}>` : '@Design-Lead';
    mentionLeads += ' and ';
    mentionLeads += qaLeadId ? `<@${qaLeadId}>` : '@QA-Lead';
  }
...
async function main() {
  ...
  const performAudit = () => {
    ...
    return [...accessibilityIssues, ...colorIssues, ...buttonIssues, ...buttonStyleIssues, ...styleIssues];
  };

  let allIssues = performAudit();

  if (process.argv.includes('--fix')) {
    applyAutoFixes(allIssues);
    // Re-scan after fixes
    allIssues = performAudit();
  }
