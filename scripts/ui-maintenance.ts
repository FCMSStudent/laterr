...
/**
 * Consolidated scanner for accessibility issues (missing labels, aria-labels, alt text)
 */
function scanAccessibilityIssues(): UIIssue[] {
  ...
  try {
    ...
    for (const file of files) {
      ...
      // 2. Form elements (input, select, textarea) missing labels
      ...
      while ((match = formRegex.exec(content)) !== null) {
        ...
        // Skip certain types
        if (attributes.includes('type="hidden"') ||
            attributes.includes("type='hidden'") ||
            attributes.includes('type="submit"') ||
            attributes.includes('type="button"')) continue;
        ...
        // Check for nested labels (heuristic)
        if (!hasLabel) {
          const beforeContent = content.substring(Math.max(0, match.index - 200), match.index);
          const lastLabelOpen = Math.max(beforeContent.lastIndexOf('<label'), beforeContent.lastIndexOf('<Label'));
          const lastLabelClose = Math.max(beforeContent.lastIndexOf('</label>'), beforeContent.lastIndexOf('</Label>'));
            ...
            for (const result of testItem.results || []) {
              if (result.status === 'failed') {
                ...
                for (const fail of failures) {
                  ...
                  failureMessage = failureMessage.replace(/\x1b\[[0-9;]*m/g, '');

                  // Sanitize paths
                  failureMessage = failureMessage.replace(new RegExp(process.cwd(), 'g'), '[PATH]');
...
/**
 * Automatically fixes minor UI issues using a bottom-up approach to preserve offsets
 */
...
/**
 * Generates a human-readable Markdown report, grouping identical issues
 */
...
async function sendSlackNotification(issues: UIIssue[]) {
  ...
  if (criticalIssues.length > 0) {
    mentionLeads = '\n🚨 *Critical Design/Accessibility Blockers Found!*';
    if (designLeadId || qaLeadId) {
      mentionLeads += ' tagging';
      if (designLeadId) mentionLeads += ` <@${designLeadId}>`;
      if (qaLeadId) mentionLeads += ` <@${qaLeadId}>`;
    } else {
      mentionLeads += ' tagging @Design-Lead and @QA-Lead';
    }
  }
...
async function main() {
  ...
  const performScan = () => {
    ...
    return [...accessibilityIssues, ...colorIssues, ...buttonIssues, ...styleIssues];
  };
  ...
  if (process.argv.includes('--fix')) {
    ...
    // Re-scan after fixes to get accurate final counts
