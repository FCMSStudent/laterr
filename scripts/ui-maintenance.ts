...
/**
 * Scans for accessibility issues including missing labels for form elements and icon-only buttons
 */
function scanAccessibility(): UIIssue[] {
  ...
  try {
    ...
    for (const file of files) {
      ...
      // 1. Scan for form elements (input, select, textarea)
      ...
      while ((match = formRegex.exec(content)) !== null) {
        ...
        // Skip hidden inputs, submit buttons, and standard buttons
...
function runRuntimeAudit(): UIIssue[] {
  ...
  try {
    // Ensure we only run chromium to avoid missing browser errors in CI/sandbox
    // and use a clean redirection that doesn't include potential stderr noise
    if (fs.existsSync(resultsPath)) fs.unlinkSync(resultsPath);

    try {
      execSync(`npx playwright test tests/ui-audit.spec.ts --project=chromium --reporter=json > ${resultsPath} 2>&1 || true`);
    } catch (e) {
      // execSync might throw even with || true if it fails to even start
      console.warn('Playwright execution returned an error (expected if tests failed)');
    }
    ...
    if (fs.existsSync(resultsPath)) {
      ...
      // Sometimes there's noise before or after the JSON if reporter=json is used with redirection
      const jsonStart = content.indexOf('{');
      const jsonEnd = content.lastIndexOf('}');
      if (jsonStart === -1 || jsonEnd === -1) {
        throw new Error('Could not find valid JSON in Playwright output');
      }

      const results: PlaywrightResult = JSON.parse(content.substring(jsonStart, jsonEnd + 1));
            ...
            for (const result of test.results || []) {
              if (result.status === 'failed') {
                ...
                for (const fail of failures) {
                  const rawMessage = fail.message || 'Unknown failure';
                  // Sanitize error message to remove long paths and make it readable
...
async function sendSlackNotification(issues: UIIssue[]) {
  ...
  const designLeadId = process.env.DESIGN_LEAD_SLACK_ID || '<@Design-Lead>';
  const qaLeadId = process.env.QA_LEAD_SLACK_ID || '<@QA-Lead>';
...
async function main() {
  ...
  let allIssues = [...accessibilityIssues, ...altIssues, ...colorIssues, ...buttonIssues, ...styleIssues];

  if (process.argv.includes('--fix')) {
    applyAutoFixes(allIssues);
    // Re-scan after fixes to get accurate report
    const accessibilityIssuesPost = scanAccessibility();
    const altIssuesPost = scanMissingAlt();
    const colorIssuesPost = scanHardcodedColors();
    const buttonIssuesPost = scanInconsistentButtons();
    const styleIssuesPost = scanInconsistentStyles();
    allIssues = [...accessibilityIssuesPost, ...altIssuesPost, ...colorIssuesPost, ...buttonIssuesPost, ...styleIssuesPost];
  }
