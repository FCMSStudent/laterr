...
/**
 * Scans for accessibility issues: missing alt attributes and form labels
 */
...
async function sendSlackNotification(issues: UIIssue[]) {
  ...
  const designLead = process.env.DESIGN_LEAD_SLACK_ID;
  const qaLead = process.env.QA_LEAD_SLACK_ID;

  let mentionLeads = '';
  if (criticalIssues.length > 0) {
    const mentions = [
      designLead ? `<@${designLead}>` : '@Design-Lead',
      qaLead ? `<@${qaLead}>` : '@QA-Lead'
    ].join(' and ');
    mentionLeads = `\n🚨 *Critical Design/Accessibility Blockers Found!* tagging ${mentions}`;
  }
...
async function main() {
  ...
  if (process.argv.includes('--fix')) {
    ...
    // Re-scan after fixes if needed, but for now we just report what we found and what we fixed.
  }
