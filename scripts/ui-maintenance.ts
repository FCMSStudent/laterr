...
/**
 * Scans for accessibility issues in form elements and buttons
 */
        ...
        if (!attributes.includes('aria-label') && !attributes.includes('aria-labelledby')) {
          ...
          if (hasIcon && !textContent) {
            ...
            issues.push({
              type: 'accessibility',
              severity: 'high',
              description: `<${tag}> icon-only button missing aria-label`,
              file,
              line: lineNum,
              autoFixable: false
            });
    ...
    if (fontOutput) {
      fontOutput.split('\n').filter(Boolean).forEach(line => {
        ...
        issues.push({
          type: 'styling',
          severity: 'low',
          description: `Hardcoded font found: ${description}`,
          file,
          line: parseInt(lineNum),
          autoFixable: false
        });
...
function runRuntimeAudit(): UIIssue[] {
  ...
  try {
    // Sanitize Playwright output to handle potential npm warnings and strip ANSI colors
    ...
    if (jsonStart !== -1) {
      const sanitizedOutput = output.substring(jsonStart)
...
function generateMarkdownReport(issues: UIIssue[]) {
  ...
  for (const sev of severities) {
    const sevIssuesKeys = Object.keys(groupedIssues).filter(k => groupedIssues[k][0].severity === sev);
    if (sevIssuesKeys.length > 0) {
      md += `### ${sev.toUpperCase()}\n\n`;
      md += `| Type | Description | Files |\n`;
      md += `| --- | --- | --- |\n`;
      for (const key of sevIssuesKeys) {
        const group = groupedIssues[key];
        const first = group[0];
        const files = group.map(i => `\`${i.file || 'runtime'}\`${i.line ? `:${i.line}` : ''}`).join(', ');
        md += `| ${first.type} | ${first.description} | ${files} |\n`;
      }
      md += `\n`;
    }
  }
...
async function main() {
  ...
  if (process.argv.includes('--fix')) {
    ...
    // Re-scan after fixes to ensure final counts are accurate
    const accessibilityIssuesPost = scanAccessibilityIssues();
