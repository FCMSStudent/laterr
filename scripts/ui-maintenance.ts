...
/**
 * Scans for accessibility issues (icon-only buttons and missing form labels)
 */
function scanAccessibilityIssues(): UIIssue[] {
  ...
  try {
    ...
    for (const file of files) {
      ...
      // Skip shared UI components as they are primitives
      if (!isSharedUI) {
    ...
    for (const file of files) {
      ...
      while ((match = hexRegex.exec(content)) !== null) {
        ...
        if (lineContent.includes('var(') ||
            lineContent.includes(`[stroke='${hex}']`) ||
            lineContent.includes(`[fill='${hex}']`) ||
            lineContent.includes(`stroke="${hex}"`) ||
            lineContent.includes(`fill="${hex}"`)) continue;
...
async function main() {
  ...
  let allIssues = [...altIssues, ...accessibilityIssues, ...colorIssues, ...buttonIssues, ...styleIssues];

  if (process.argv.includes('--fix')) {
    applyAutoFixes(allIssues);
    // Re-scan after fixes to get updated report
    const updatedAccessibility = scanAccessibilityIssues();
    const updatedAlt = scanMissingAlt();
    const updatedColors = scanHardcodedColors();
    allIssues = [...updatedAlt, ...updatedAccessibility, ...updatedColors, ...buttonIssues, ...styleIssues];
  }
