...
/**
 * Scans for accessibility issues (missing labels, alt text, etc.)
 */
function scanAccessibility(): UIIssue[] {
  ...
  try {
    ...
    for (const file of files) {
      ...
      // 2. Scan for buttons (missing aria-labels)
...
function runRuntimeAudit(): UIIssue[] {
  ...
  try {
    // Run only chromium to speed up and avoid potential issues with other browsers
    execSync(`npx playwright test tests/ui-audit.spec.ts --project=chromium --reporter=json > ${resultsPath} 2>/dev/null || true`);

    if (fs.existsSync(resultsPath)) {
      let resultsText = fs.readFileSync(resultsPath, 'utf8');
      // If there's non-JSON output at the beginning (like npm warnings), try to find the first '{'
      if (resultsText.trim() && !resultsText.trim().startsWith('{')) {
        const firstBrace = resultsText.indexOf('{');
        if (firstBrace !== -1) {
          resultsText = resultsText.substring(firstBrace);
        }
      }
      const results: PlaywrightResult = JSON.parse(resultsText);
...
async function main() {
  ...
  if (process.argv.includes('--fix')) {
    ...
    // After fixing, re-scan to get an accurate updated issue count (some might still be unfixable or new ones might be found)
    const updatedAccessibility = scanAccessibility();
    const updatedColor = scanHardcodedColors();
    allIssues = [...updatedAccessibility, ...updatedColor, ...buttonIssues, ...styleIssues];
  }
