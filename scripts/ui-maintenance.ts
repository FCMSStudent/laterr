...
/**
 * Scans for accessibility issues: missing form labels and icon-only buttons
 */
function scanAccessibility(): UIIssue[] {
  ...
  try {
    ...
    for (const file of files) {
      ...
      // 1. Scan for form elements (input, select, textarea, etc.)
            ...
            for (const result of test.results || []) {
              if (result.status === 'failed') {
                ...
                for (const fail of failures) {
                  const failureMessage = (fail.message || 'Unknown failure').replace(process.cwd(), '[PATH]');
                  issues.push({
                    type: 'accessibility',
...
async function main() {
  ...
  if (process.argv.includes('--fix')) {
    ...
    // Re-scan after fixes to get accurate unfixable count for the report
