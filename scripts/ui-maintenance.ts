...
/**
 * Scans for accessibility issues like missing form labels and icon-only buttons
 */
function scanAccessibility(): UIIssue[] {
  const issues: UIIssue[] = [];
  try {
    const files = execSync('find src -name "*.tsx"').toString().split('\n').filter(Boolean);
    const excludePattern = "src/shared/components/ui";

    for (const file of files) {
      const content = fs.readFileSync(file, 'utf8');
      const isExcluded = file.includes(excludePattern);

      // 1. Scan for buttons (Icon-only without labels)
      ...
      // 2. Scan for form elements (inputs, selects, textareas)
      if (!isExcluded) {
    ...
    console.error('Error scanning for accessibility:', e);
  }
  return issues;
}
...
function runRuntimeAudit(): UIIssue[] {
  ...
  try {
    // Run only chromium to speed up and avoid noise
            ...
            for (const result of test.results || []) {
              if (result.status === 'failed') {
                ...
                for (const fail of failures) {
                  ...
                  const sanitizedMessage = failureMessage.replace(/\/home\/jules\/[^ ]+/g, '[PATH]');

                  issues.push({
                    type: 'accessibility',
                    severity: failureMessage.toLowerCase().includes('accessibility') ? 'high' : 'critical',
                    description: `Runtime audit failure in "${spec.title}": ${sanitizedMessage}`,
                    autoFixable: false
                  });
                }
              }
...
function applyAutoFixes(issues: UIIssue[]) {
  ...
  for (const [file, fileIssues] of Object.entries(fileGroups)) {
    try {
      ...
      // Sort by offset descending to avoid shifting issues when replacing
...
async function main() {
  ...
  const accIssues = scanAccessibility();
  ...
  let allIssues = [...accIssues, ...altIssues, ...colorIssues, ...buttonIssues, ...styleIssues];

  if (process.argv.includes('--fix')) {
    applyAutoFixes(allIssues);
    // Re-scan after fixes to get updated state for the report
    const updatedAcc = scanAccessibility();
    const updatedAlt = scanMissingAlt();
    const updatedColor = scanHardcodedColors();
    allIssues = [...updatedAcc, ...updatedAlt, ...updatedColor, ...buttonIssues, ...styleIssues];
  }
