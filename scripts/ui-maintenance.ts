...
/**
 * Scans for accessibility issues (alt attributes, form labels, icon buttons)
 */
function scanAccessibilityIssues(): UIIssue[] {
  ...
  try {
    ...
    for (const file of files) {
      ...
      // 2. Scan for icon-only buttons missing labels
      ...
      // 3. Scan for form elements missing accessible labels (refactored for robustness)
      const formRegex = /<(input|select|textarea|Input|Select|Textarea)([^>]*?)(\/?>)/g;
      while ((match = formRegex.exec(content)) !== null) {
        const tag = match[1];
        const attributes = match[2];

        // Skip non-form controls
        ...
        // Search for associated label in the entire file content
        ...
        if (!hasAriaLabel && !hasAssociatedLabel) {
          // Heuristic: check if nested in <label> or <Label>
          const beforeTag = content.substring(Math.max(0, match.index - 300), match.index);
          ...
          const isNestedInLabel = lastLabelOpen > lastLabelClose;
...
/**
 * Scans for custom Tailwind styling overrides on the standard Button component
 */
  ...
  try {
    ...
    for (const file of files) {
      ...
      while ((match = buttonRegex.exec(content)) !== null) {
        ...
        const hasOverrides = /\b(bg-|text-|p-|m-|rounded-|border-)/.test(className);
        ...
        if (hasOverrides) {
          ...
          issues.push({
            type: 'styling',
            severity: 'low',
            description: `Button component has custom styling overrides: "${className}"`,
            file,
            line: lineNum,
            autoFixable: false
          });
  ...
  try {
    ...
    if (pixelOutput) {
      pixelOutput.split('\n').filter(Boolean).forEach(line => {
        ...
        if (file && lineNum && !file.includes('src/shared/components/ui')) {
          issues.push({
            type: 'styling',
            severity: 'low',
            ...
            for (const result of test.results || []) {
              if (result.status === 'failed') {
                ...
                for (const fail of failures) {
                  const failureMessage = (fail.message || 'Unknown failure').replace(/\x1b\[[0-9;]*m/g, ''); // Strip ANSI codes
                  issues.push({
                    type: 'accessibility',
...
async function main() {
  ...
  let allIssues = [...accessibilityIssues, ...colorIssues, ...buttonIssues, ...buttonStyleIssues, ...styleIssues];

  if (process.argv.includes('--fix')) {
    applyAutoFixes(allIssues);
    // Optionally re-scan after fixes
    const accessibilityIssuesAfter = scanAccessibilityIssues();
    const colorIssuesAfter = scanHardcodedColors();
    allIssues = [...accessibilityIssuesAfter, ...colorIssuesAfter, ...buttonIssues, ...buttonStyleIssues, ...styleIssues];
  }
