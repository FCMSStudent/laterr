...
/**
 * Scans for missing accessible labels on form elements and icon-only buttons
 */
...
/**
 * Scans for direct Tailwind overrides on Button components
 */
  ...
  try {
    ...
    for (const file of files) {
      ...
      while ((match = buttonRegex.exec(content)) !== null) {
        ...
        if (classNames.includes('bg-') || classNames.includes('text-') || classNames.includes('p-')) {
          const lineNum = content.substring(0, match.index).split('\n').length;
          issues.push({
            type: 'styling',
            severity: 'low',
            description: `Custom styling override on <Button>: "${classNames}"`,
            file,
            line: lineNum,
            autoFixable: false
          });
    ...
    console.error('Error scanning for button style overrides:', e);
  }
  return issues;
}
...
function runRuntimeAudit(): UIIssue[] {
  ...
  try {
    // Only run chromium and strip ANSI colors from output
    ...
    const sanitizedOutput = output.replace(/\x1b\[[0-9;]*[a-zA-Z]/g, '');
    const jsonStart = sanitizedOutput.indexOf('{');
    if (jsonStart === -1) return [];

    const jsonOutput = sanitizedOutput.substring(jsonStart);
    fs.writeFileSync(resultsPath, jsonOutput);
            ...
            for (const result of test.results || []) {
              if (result.status === 'failed') {
                ...
                for (const fail of failures) {
                  ...
                  // Sanitize error message: mask paths and truncate
                  failureMessage = failureMessage.replace(/\/[^ ]+\//g, '[PATH]/')
                                               /* eslint-disable-next-line no-control-regex */
...
function generateMarkdownReport(issues: UIIssue[]) {
  ...
  for (const sev of severities) {
    ...
    if (sevIssues.length > 0) {
      ...
      for (const [key, instances] of Object.entries(groupedIssues)) {
        const [type, description] = key.split('|');
        if (instances.length > 3) {
           md += `| ${type} | ${description} (${instances.length} occurrences) | \`${instances[0].file || 'runtime'}\` | Multiple |\n`;
        } else {
...
async function main() {
  ...
  const performScan = () => {
    ...
    return [...accessibilityIssues, ...altIssues, ...colorIssues, ...buttonIssues, ...styleIssues, ...buttonStyleIssues];
  };
  ...
  if (process.argv.includes('--fix')) {
    ...
    // Re-scan after fixes to ensure report accuracy
    console.log('Re-scanning after fixes...');
