...
/**
 * Scans for accessibility issues (alt text, form labels, icon buttons)
 */
function scanAccessibility(): UIIssue[] {
  ...
  try {
    ...
    for (const file of files) {
      ...
      // 1. Scan for missing alt attributes on images
      ...
      while ((imgMatch = imgRegex.exec(content)) !== null) {
        ...
        if (!tag.includes('alt=')) {
          ...
          issues.push({
            ...
            line: lineNum,
            autoFixable: true,
            offset: imgMatch.index,
            originalText: tag
          });
        }
      }

      // Skip detailed form/button scans for library components to reduce noise
      if (isSharedUI) continue;

      // 2. Scan for missing form labels (input, select, textarea)
      const formRegex = /<(input|select|textarea|Input|Select|Textarea)([^>]*?)(\/?>)/g;
      let formMatch;
      while ((formMatch = formRegex.exec(content)) !== null) {
        const tag = formMatch[1];
        const attributes = formMatch[2];
        ...
        if (!hasAriaLabel && !hasAssociatedLabel) {
          // Check if it's wrapped in a <label> (heuristic)
          const beforeTag = content.substring(Math.max(0, formMatch.index - 200), formMatch.index);
          ...
          if (!isNestedInLabel) {
            const lineNum = content.substring(0, formMatch.index).split('\n').length;
            issues.push({
              type: 'accessibility',
              severity: 'high',
      ...
      // 3. Scan for icon-only buttons missing labels
      const buttonRegex = /<(Button|button)([^>]*?)>([\s\S]*?)<\/\1>/g;
      let buttonMatch;
      while ((buttonMatch = buttonRegex.exec(content)) !== null) {
        const attributes = buttonMatch[2];
        const children = buttonMatch[3];
...
function runRuntimeAudit(): UIIssue[] {
  ...
  try {
    // Run only chromium to speed up and avoid potential issues with other browsers in sandbox
              ...
              if (result.status === 'failed') {
                ...
                for (const fail of failures) {
                  const failureMessage = (fail.message || 'Unknown failure')
                    .replace(/\/[^ ]+/g, '[PATH]') // Sanitize paths
                    .split('\n').slice(0, 3).join(' '); // Truncate
  ...
  for (const [file, fileIssues] of Object.entries(fileGroups)) {
    try {
      ...
      if (modified) {
        ...
        console.log(`Applied auto-fixes to ${file}`);
      }
    } catch (e) {
...
async function main() {
  ...
  if (process.argv.includes('--fix')) {
    ...
    // Re-scan after fixes to get updated report
