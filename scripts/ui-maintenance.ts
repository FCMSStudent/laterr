...
/**
 * Scans for accessibility issues such as missing form labels and aria-labels on icon buttons
 */
...
function runRuntimeAudit(): UIIssue[] {
  ...
  try {
    ...
    if (jsonStart !== -1) {
      const results: PlaywrightResult = JSON.parse(output.substring(jsonStart));
              ...
              if (result.status === 'failed') {
                ...
                for (const fail of failures) {
                  const failureMessage = (fail.message || 'Unknown failure')
                    ...
                    .replace(/\/[^ ]+\//g, '[PATH]/'); // Sanitize absolute paths
                  issues.push({
                    type: 'accessibility',
