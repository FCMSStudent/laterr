...
function scanAccessibility(): UIIssue[] {
  ...
  try {
    ...
    for (const file of files) {
      ...
      // 1. Scan for icon-only buttons missing labels
      ...
      // Including capitalized variants for common UI components
      ...
      while ((match = formElementRegex.exec(content)) !== null) {
        ...
        if (!hasAriaLabel && !hasAssociatedLabel) {
            // Check if it's wrapped in a <label> (lookback heuristic)
...
function runRuntimeAudit(): UIIssue[] {
  ...
  try {
    // Only run chromium as per standard and redirect stderr to avoid polluting output
    ...
    if (fs.existsSync(resultsPath)) {
      ...
      // Parse for the first occurrence of '{' to handle potential npm warnings or garbage in the output
      const jsonStart = rawResults.indexOf('{');
      if (jsonStart === -1) {
        console.warn('No JSON object found in runtime audit results.');
        return issues;
      }

      const results: PlaywrightResult = JSON.parse(rawResults.substring(jsonStart));
...
async function main() {
  ...
  if (process.argv.includes('--fix')) {
    ...
    // Re-scan after fixes to get accurate final report
    const accessibilityIssuesAfter = scanAccessibility();
    ...
    allIssues = [...accessibilityIssuesAfter, ...altIssuesAfter, ...colorIssuesAfter, ...buttonIssuesAfter, ...styleIssuesAfter];
  }
