...
/**
 * Robust scanner for accessibility issues: missing alt, icon-only buttons, and form labels
 */
function scanAccessibility(): UIIssue[] {
  const issues: UIIssue[] = [];
  try {
    const files = execSync('find src -name "*.tsx"').toString().split('\n').filter(Boolean);
    const appFiles = files.filter(f => !f.includes('src/shared/components/ui'));
    ...
    for (const file of files) {
      ...
      // 2. Scan for icon-only buttons and form labels (skip UI library components for these)
      if (!file.includes('src/shared/components/ui')) {
        ...
        while ((match = formRegex.exec(content)) !== null) {
          ...
          if (attributes.includes('type="hidden"') || attributes.includes("type='hidden'") || attributes.includes('type="submit"') || attributes.includes('type="button"')) continue;
...
async function main() {
  ...
  if (process.argv.includes('--fix')) {
    ...
    allIssues = [...accessibilityIssuesPost, ...colorIssuesPost, ...buttonIssues, ...styleIssues];
  }
