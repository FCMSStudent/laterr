...
/**
 * Scans for various accessibility issues: missing alt attributes, icon-only buttons without labels,
 * and form elements missing accessible labels.
 */
function scanAccessibilityIssues(): UIIssue[] {
  ...
  try {
    ...
    for (const file of files) {
      ...
      // 1. Missing alt attributes on <img>
      ...
      // 3. Form elements (input, select, textarea) missing labels
      ...
      while ((match = formRegex.exec(content)) !== null) {
        ...
        if (!hasAriaLabel && !hasAssociatedLabel) {
          ...
          const isNestedInLabel = (beforeTag.toLowerCase().includes('<label') && !beforeTag.toLowerCase().includes('</label>'));
...
function scanInconsistentStyles(): UIIssue[] {
  const issues: UIIssue[] = [];
  try {
    const pixelOutput = execSync('grep -rnE "(p|m|gap|w|h|top|left|right|bottom)-\\[[0-9]+px\\]" src/ --include="*.tsx" | grep -v "src/shared/components/ui" || true').toString();
    if (pixelOutput) {
    ...
    const fontOutput = execSync('grep -rn "font-\\[" src/ --include="*.tsx" | grep -v "src/shared/components/ui" || true').toString();
    if (fontOutput) {
      fontOutput.split('\n').filter(Boolean).forEach(line => {
        const [file, lineNum, ...rest] = line.split(':');
        issues.push({
...
function runRuntimeAudit(): UIIssue[] {
  ...
  try {
    // Suppress ANSI colors and parse the first valid JSON block
    ...
    if (jsonStartIndex === -1) {
        console.error('No valid JSON output from Playwright audit.');
        return issues;
    }

    const cleanJson = output.substring(jsonStartIndex);
    const results: PlaywrightResult = JSON.parse(cleanJson);
          ...
          for (const result of test.results || []) {
            if (result.status === 'failed') {
              ...
              for (const fail of failures) {
                const failureMessage = (fail.message || 'Unknown failure').replace(/\x1b\[[0-9;]*m/g, ''); /* eslint-disable-line no-control-regex */
                issues.push({
                  type: 'accessibility',
                  severity: failureMessage.toLowerCase().includes('accessibility') || failureMessage.toLowerCase().includes('axe') ? 'high' : 'critical',
...
function applyAutoFixes(issues: UIIssue[]) {
  ...
  for (const [file, fileIssues] of Object.entries(fileGroups)) {
    try {
      ...
      // Sort by offset descending to avoid shifting indices
...
function generateMarkdownReport(issues: UIIssue[]) {
  ...
  for (const sev of severities) {
    ...
    if (sevIssues.length > 0) {
      ...
      // Group identical issues to reduce noise
      const groupedIssues: Record<string, UIIssue[]> = {};
      ...
      for (const group of Object.values(groupedIssues)) {
        const first = group[0];
        if (group.length > 5) {
          md += `| ${first.type} | ${first.description} | *Multiple (${group.length} files)* | - |\n`;
        } else {
...
async function sendSlackNotification(issues: UIIssue[]) {
  ...
  const designLeadId = process.env.DESIGN_LEAD_SLACK_ID;
  const qaLeadId = process.env.QA_LEAD_SLACK_ID;
  ...
  if (criticalIssues.length > 0) {
    const designTag = designLeadId ? `<@${designLeadId}>` : '@Design-Lead';
    const qaTag = qaLeadId ? `<@${qaLeadId}>` : '@QA-Lead';
    mentionLeads = `\n🚨 *Critical Design/Accessibility Blockers Found!* tagging ${designTag} and ${qaTag}`;
  }
...
async function main() {
  ...
  const performScan = () => [
    ...
    ...scanInconsistentButtons(),
    ...scanInconsistentStyles(),
    ...scanHardcodedColors()
  ];

  let allIssues = performScan();

  if (process.argv.includes('--fix')) {
    applyAutoFixes(allIssues);
    // Re-scan after fixes to have accurate report
    allIssues = performScan();
  }
