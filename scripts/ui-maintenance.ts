...
function scanAccessibilityIssues(): UIIssue[] {
  ...
  try {
    ...
    for (const file of files) {
      ...
      // 1. Scan for buttons (standard and raw)
      ...
      // 2. Scan for inputs, selects, textareas (standard and raw)
...
function runRuntimeAudit(): UIIssue[] {
  ...
  try {
    // Ensure port is available before running
...
async function sendSlackNotification(issues: UIIssue[]) {
  ...
  if (criticalIssues.length > 0) {
    ...
    if (!designLeadId && !qaLeadId) mentionLeads += '@Design-Lead and @QA-Lead';
  }
