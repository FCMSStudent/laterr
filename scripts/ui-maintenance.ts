...
/**
 * Scans for missing form labels and inaccessible elements
 */
function scanAccessibilityIssues(): UIIssue[] {
  ...
  try {
    ...
    for (const file of files) {
      ...
      // 2. Scan for missing form labels on input, select, textarea
...
function generateMarkdownReport(issues: UIIssue[]) {
  ...
  for (const sev of severities) {
    ...
    if (sevIssues.length > 0) {
      ...
      // Group identical issues for clarity
      const grouped: Record<string, { type: string; description: string; locations: string[] }> = {};
      for (const issue of sevIssues) {
        ...
        const loc = issue.file ? `\`${issue.file}\`${issue.line ? `:${issue.line}` : ''}` : 'runtime';
        if (!grouped[key].locations.includes(loc)) {
          grouped[key].locations.push(loc);
        }
      }

      md += `| Type | Description | Locations |\n`;
      md += `| --- | --- | --- |\n`;
      for (const item of Object.values(grouped)) {
        const locationsStr = item.locations.length > 3
          ? `${item.locations.slice(0, 3).join(', ')} (+ ${item.locations.length - 3} more)`
          : item.locations.join(', ');
        md += `| ${item.type} | ${item.description} | ${locationsStr} |\n`;
      }
      md += `\n`;
    }
  }
...
async function main() {
  ...
  if (process.argv.includes('--fix')) {
    ...
    // Re-scan after fixes if needed, but for now we'll just report what was found
  }
