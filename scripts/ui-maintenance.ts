...
async function sendSlackNotification(issues: UIIssue[]) {
  ...
  const designLeadTag = designLeadId ? `<@${designLeadId}>` : '@Design-Lead';
  const qaLeadTag = qaLeadId ? `<@${qaLeadId}>` : '@QA-Lead';

  const mentionLeads = criticalIssues.length > 0
    ? `\n🚨 *Critical Design/Accessibility Blockers Found!* tagging ${designLeadTag} and ${qaLeadTag}`
    : '';
...
async function main() {
  ...
  if (process.argv.includes('--fix')) {
    ...
    // Re-scan after fixes to get updated counts
    const altIssuesAfter = scanMissingAlt();
    const formIssuesAfter = scanInaccessibleFormElements();
    ...
    allIssues = [...altIssuesAfter, ...formIssuesAfter, ...colorIssuesAfter, ...buttonIssuesAfter, ...styleIssuesAfter];
  }
