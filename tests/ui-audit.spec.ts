  ...
  for (const pageInfo of pages) {
    test(`Full audit for ${pageInfo.name}`, async ({ page }) => {
      ...
      try {
        ...
        // Throwing will mark the test as failed which is what we want for the report
        throw e;
      }
  ...
  test('Check for overlapping and misaligned elements', async ({ page }) => {
    ...
    for (let i = 0; i < elementsData.length; i++) {
      ...
      if (viewport && b1.x + b1.width > viewport.width) {
        const msg = `Element extends beyond viewport width: <${e1.tagName}> "${e1.text}" is at x=${b1.x} with width=${b1.width} (viewport width=${viewport.width})`;
      ...
      if (b1.width < 44 || b1.height < 44) {
        const msg = `Small touch target detected: <${e1.tagName}> "${e1.text}" is ${b1.width}x${b1.height} at (${b1.x}, ${b1.y})`;
        expect.soft(b1.width, msg).toBeGreaterThanOrEqual(44);
        expect.soft(b1.height, msg).toBeGreaterThanOrEqual(44);
      }

      // Check for text overflow in e1
      if (e1.hasOverflow) {
        const msg = `Potential text overflow in <${e1.tagName}>: "${e1.text}..."`;
      ...
      for (let j = i + 1; j < elementsData.length; j++) {
        ...
        if (intersectionArea > 0) {
          ...
          if (overlapPercent > 0.5) { // If 50% of the smaller element is covered
             const msg = `Significant overlap (${Math.round(overlapPercent * 100)}%) between <${e1.tagName}> "${e1.text}" and <${e2.tagName}> "${e2.text}"`;
        ...
        if (horizontalGap < 50 && verticalDiff > 0 && verticalDiff < 5) {
          const msg = `Potential misalignment: <${e1.tagName}> "${e1.text}" and <${e2.tagName}> "${e2.text}" are vertically offset by ${verticalDiff}px`;
