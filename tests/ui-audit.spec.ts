...
test.describe('UI Accessibility and Layout Audit', () => {
  const pages = [
    { name: 'Landing', path: '/' },
  ...
  for (const pageInfo of pages) {
    test(`Full audit for ${pageInfo.name}`, async ({ page }) => {
      ...
      // Network request failure check
      expect(
        failedRequests,
        `Found ${failedRequests.length} failed network requests on ${pageInfo.name}`,
      ).toHaveLength(0);

      // Layout check (overlap and overflow)
      const viewport = page.viewportSize();
      const elementsData = await page.evaluate(() => {
        const els = Array.from(document.querySelectorAll('button, a, input, [role="button"], select, textarea')).filter((el) => {
