...
test.describe('UI Accessibility Audit', () => {
  ...
  for (const pageInfo of pages) {
    test(`Full audit for ${pageInfo.name}`, async ({ page }) => {
      ...
      // Check for overlapping and misaligned elements
      ...
      for (let i = 0; i < bboxes.length; i++) {
        ...
        for (let j = i + 1; j < bboxes.length; j++) {
          ...
          if (horizontalGap < 50 && verticalDiff > 0 && verticalDiff < 5) {
             const msg = `Potential misalignment on ${pageInfo.name}: <${tagName1}> and <${tagName2}> are vertically offset by ${verticalDiff}px`;
