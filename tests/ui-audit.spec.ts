  ...
  for (const pageInfo of pages) {
    test(`Full audit for ${pageInfo.name}`, async ({ page }) => {
      ...
      try {
        ...
        // We throw to fail the test but the summary will be captured
        throw e;
      }
      ...
      const elementsData = await page.evaluate(() => {
        const els = Array.from(document.querySelectorAll('button, a, input, [role="button"], img'))
          .filter(el => {
      ...
      for (let i = 0; i < elementsData.length; i++) {
        ...
        // Check for small touch targets (less than 44x44) for interactive elements
        if (e1.tagName !== 'img' && (b1.width < 44 || b1.height < 44)) {
          const msg = `Small touch target detected on ${pageInfo.name}: <${e1.tagName}> "${e1.text}" is ${b1.width}x${b1.height} at (${b1.x}, ${b1.y})`;
          expect.soft(b1.width >= 44 && b1.height >= 44, msg).toBeTruthy();
        }
