      ...
      for (let j = i + 1; j < elementsData.length; j++) {
        ...
        if (intersectionArea > 0) {
          ...
          if (overlapPercent > 0.5) { // If 50% of the smaller element is covered
             const msg = `Significant element overlap detected (${Math.round(overlapPercent * 100)}%) between <${e1.tagName}> "${e1.text}" and <${e2.tagName}> "${e2.text}"`;
        ...
        if (horizontalGap < 50 && verticalDiff > 0 && verticalDiff < 5) {
          const msg = `Potential misalignment: <${e1.tagName}> and <${e2.tagName}> are vertically offset by ${verticalDiff}px`;
