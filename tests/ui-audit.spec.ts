...
test.describe('UI Accessibility Audit', () => {
  ...
  test('Check for overlapping and misaligned elements', async ({ page }) => {
    const pageName = 'Components Showcase';
    ...
    for (let i = 0; i < elementsData.length; i++) {
      ...
      if (viewport && b1.x + b1.width > viewport.width) {
        const msg = `Element extends beyond viewport width on ${pageName}: <${e1.tagName}> "${e1.text}" is at x=${b1.x} with width=${b1.width} (viewport width=${viewport.width})`;
      ...
      if (b1.width < 44 || b1.height < 44) {
        const msg = `Small touch target detected on ${pageName}: <${e1.tagName}> "${e1.text}" is ${b1.width}x${b1.height} at (${b1.x}, ${b1.y})`;
      ...
      if (e1.hasOverflow) {
        const msg = `Potential text overflow on ${pageName} in <${e1.tagName}>: "${e1.text}..."`;
      ...
      for (let j = i + 1; j < elementsData.length; j++) {
        ...
        if (intersectionArea > 0) {
          ...
          if (overlapPercent > 0.5) { // If 50% of the smaller element is covered
             const msg = `Significant element overlap detected (${Math.round(overlapPercent * 100)}%) on ${pageName} between <${e1.tagName}> "${e1.text}" and <${e2.tagName}> "${e2.text}"`;
        ...
        if (horizontalGap < 50 && verticalDiff > 0 && verticalDiff < 5) {
          const msg = `Potential misalignment on ${pageName}: <${e1.tagName}> "${e1.text}" and <${e2.tagName}> "${e2.text}" are vertically offset by ${verticalDiff}px`;
