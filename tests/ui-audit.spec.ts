...
test.describe('UI Accessibility Audit', () => {
  ...
  for (const pageInfo of pages) {
    test(`Layout audit for ${pageInfo.name} (overlaps, misalignments, overflows)`, async ({ page }) => {
      await page.goto(pageInfo.path);
      await page.waitForLoadState('networkidle');
      ...
      for (let i = 0; i < elementsData.length; i++) {
        ...
        if (elementsData[i].hasOverflow) {
          const msg = `Potential text overflow on ${pageInfo.name} in <${tagName1}>: "${text1}..."`;
        ...
        for (let j = i + 1; j < elementsData.length; j++) {
          ...
          if (intersectionArea > 0) {
            ...
            if (overlapPercent > 0.5) {
              const msg = `Significant element overlap detected on ${pageInfo.name} (${Math.round(overlapPercent * 100)}%) between <${tagName1}> "${text1}" and <${tagName2}> "${text2}"`;
